...
Read the assistant prompt at ${promptPath} first, then read this space's INSTRUCTION.md and inspect relevant notes in the same space. Produce one concise Markdown draft for the user to review. Do not overwrite existing canonical notes directly. If you write a file, write it under ${spaceDraftsPath(spacePath)} with a dated filename. Keep the draft focused, source-aware, and easy to accept or discard.`,
  ...
  inbox: {
    ...
    reviewPageSubtitle: 'Select captures for the Inbox Organizer.',
    ...
    captureNoAiHint: 'Assistant waits for Review',
    ...
    organizationAssistantPreviewDesc: 'This assistant helps clear the queue when you open Review.',
    organizationAssistantWorkbenchDesc: 'Choose the captures this assistant should organize. You confirm proposed changes before anything is written.',
    ...
    sourcePreviewActiveDesc: 'This is staged as raw source. The assistant waits for Review.',
...
请先读取助手 Prompt：${promptPath}，再读取这个空间的 INSTRUCTION.md，并查看同一空间内相关笔记。请产出一份简洁的 Markdown 草稿供用户确认，不要直接覆盖正式笔记。如果需要写入文件，请写到 ${spaceDraftsPath(spacePath)} 下，并使用带日期的文件名。草稿要聚焦、可追溯，并方便用户确认或丢弃。`,
  ...
  inbox: {
    ...
    reviewPageSubtitle: '选择待处理材料，交给收集箱整理助手整理。',
    ...
    backToWiki: '返回 Wiki',
    queueTitle: '审核队列',
    queuePreviewDesc: '准备处理刚刚收集的材料时，继续从这里往下处理。',
    ...
    organizationAssistantPreviewDesc: '打开审核后，这个助手会帮你清理待处理队列。',
    organizationAssistantWorkbenchDesc: '选择要交给它整理的材料；写入 Mind 前会先给你确认。',
    ...
    sourcePreviewIdleDesc: '现在还没有整理动作。先收集，稍后再审核。',
    sourcePreviewActiveDesc: '这里仍是原始材料暂存，整理助手会等到审核阶段再运行。',
