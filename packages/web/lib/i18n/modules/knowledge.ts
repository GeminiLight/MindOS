...
Read the assistant prompt at ${promptPath} first, then read this space's INSTRUCTION.md and inspect relevant notes in the same space. Produce one concise Markdown draft for the user to review. Do not overwrite existing canonical notes directly. If you write a file, write it under ${spaceDraftsPath(spacePath)} with a dated filename. Keep the draft focused, source-aware, and easy to accept or discard.`,
  ...
  inbox: {
    ...
    capturePageSubtitle: 'Paste anything. Keep the source. Review later.',
    reviewPageTitle: 'Review',
    reviewPageSubtitle: 'Select captures for the Inbox Organization Agent.',
    donePageTitle: 'Done',
    ...
    captureTrustLine: 'Local first · Source preserved · Review later',
    captureInputKinds: 'Links, notes, files, drops',
    ...
    queuePreviewDesc: 'Scroll here when you are ready to clear what you captured.',
    reviewPendingAction: (n: number) => n > 0 ? `Review ${n} pending` : 'Review queue',
    organizationAgentTitle: 'Inbox Organization Agent',
    organizationAgentPreviewDesc: 'Open Review to select captures. The Agent proposes changes before writing to your Mind.',
    organizationAgentDesc: 'Select captures below. The Agent proposes changes before writing to your Mind.',
    ...
    sourcePreviewIdleDesc: 'Nothing is organized yet. Capture first, review later.',
    sourcePreviewActiveDesc: 'This is staged as raw source. The Agent waits for Review.',
    sourcePreviewEmptyTitle: 'Paste any source',
    sourcePreviewEmptyDesc: 'Links, notes, files, and dropped sources are staged here before they enter the Review queue.',
...
请先读取助手 Prompt：${promptPath}，再读取这个空间的 INSTRUCTION.md，并查看同一空间内相关笔记。请产出一份简洁的 Markdown 草稿供用户确认，不要直接覆盖正式笔记。如果需要写入文件，请写到 ${spaceDraftsPath(spacePath)} 下，并使用带日期的文件名。草稿要聚焦、可追溯，并方便用户确认或丢弃。`,
  ...
  inbox: {
    ...
    capturePageSubtitle: '随手粘贴，保留来源，稍后审核。',
    reviewPageTitle: '审核',
    reviewPageSubtitle: '选择待处理材料，交给 Inbox Organization Agent 整理。',
    donePageTitle: '完成',
    ...
    composerInputLabel: '向 Inbox 添加链接、笔记、文件或来源',
    composerPlaceholder: '粘贴链接、写下笔记，或把文件拖到这里...',
    captureTrustLine: '本地优先 · 保留来源 · 稍后审核',
    captureInputKinds: '链接、笔记、文件、拖拽',
    ...
    captureButtonCount: (n: number) => `保存 ${n} 条到 Inbox`,
    ...
    lastSavedTitle: (n: number) => `已保存 ${n} 条到 Inbox`,
    ...
    backToWiki: '返回 Wiki',
    queueTitle: '审核队列',
    queuePreviewDesc: '准备清空刚刚捕获的材料时，继续从这里往下处理。',
    reviewPendingAction: (n: number) => n > 0 ? `审核 ${n} 条待处理` : '查看审核队列',
    organizationAgentTitle: 'Inbox Organization Agent',
    organizationAgentPreviewDesc: '进入审核后选择材料，Agent 会先给整理建议，写入前由你确认。',
    organizationAgentDesc: '选择下方材料，Agent 会先给整理建议，写入前由你确认。',
    ...
    contentPreviewFailed: '预览加载失败，原始捕获仍保留在 Inbox。',
    sourcePreviewTitle: '来源预览',
    sourcePreviewIdleDesc: '现在还没有整理动作。先捕获，稍后再审核。',
    sourcePreviewActiveDesc: '这里仍是原始材料暂存，Agent 会等到审核阶段再运行。',
    sourcePreviewEmptyTitle: '随手粘贴任何来源',
    sourcePreviewEmptyDesc: '链接、笔记、文件和拖入的来源都会先暂存，之后进入审核队列。',
