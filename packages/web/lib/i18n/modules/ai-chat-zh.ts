...
export const aiChatZh = {
  ask: {
    ...
    queueFollowUpTitle: '排队发送',
    queuedFollowUpState: '排队中',
    followUpPlaceholder: '继续输入后续修改…',
    queuedFollowUpTextOnly: '文件、图片或技能需要等当前运行结束后再发送。',
    dragQueuedFollowUp: '拖拽调整顺序',
    editQueuedFollowUp: '编辑排队消息',
    saveQueuedFollowUp: '保存排队消息',
    cancelQueuedFollowUp: '取消编辑排队消息',
    ...
    reconnectingDetail: (attempt: number, max: number) => `第 ${attempt} / ${max} 次`,
    runThinking: (_runtime: string) => '正在和你一起思考',
    awaitingFirstOutput: '等待首个输出',
    toolRunningProgress: (toolName: string) => `正在执行 ${toolName}`,
    permissionWaiting: '等待你的确认',
    questionWaiting: '等待你的选择',
    pendingActionsTitle: '待处理的 Agent 请求',
    pendingActionResolvedElsewhere: '已在别处处理',
    pendingActionAnswer: '提交回答',
    pendingActionCancel: '取消',
    pendingActionApprove: '批准',
    pendingActionDeny: '拒绝',
    ...
    homeCategories: [
      {
        label: '知识管理',
        items: [
          { label: '按主题整理', desc: '把零散笔记归入合适的空间', prompt: '帮我把最近的零散笔记整理到合适的空间，并打好标签' },
    ...
    externalSessions: {
      ...
      "currentProject": "当前项目",
      "archived": "已归档",
      "hint": "此电脑上的会话。打开后，使用原 Agent 接着聊。",
      "clearSearch": "清空搜索",
      "refresh": "刷新会话",
      "loading": "正在加载会话…",
      "loadMore": "加载更多会话",
    ...
    sessionContext: {
      ...
      none: '无',
      locked: '首条消息后锁定',
      openRootInFileManager: '在文件管理器中打开工作目录',
      openRootInFileManagerFailed: '无法打开工作目录',
      editWorkDir: '设置工作目录',
  ...
  changes: {
    ...
    eyebrow: '内容审阅',
    title: '内容变更',
    subtitle: '先确认 Agent 写入，再追溯完整的内容活动记录。',
    views: {
      needsReview: '待审阅',
      allActivity: '全部活动',
    },
    ...
    pathsChanged: (n: number) => `${n} 个文件`,
    unreadCount: (n: number) => `${n} 条未读`,
    pendingReviewCount: (n: number) => `${n} 条待审阅`,
    allReviewed: '已全部审阅',
    scopedToPath: (path: string) => `限定到 ${path}`,
    scopedToSpace: (space: string) => `空间：${space}`,
    scopedToAgent: (agent: string) => `Agent：${agent}`,
    refresh: '刷新',
    markSeen: '标记已读',
    markAllRead: '全部已读',
    markReviewed: '标记已审阅',
    expandDetails: '展开变更详情',
    collapseDetails: '收起变更详情',
    diffLimitReached: '此变更过大，暂不显示差异预览。请打开文件查看内容。',
    diffPreview: 'Diff 预览',
    openFile: '打开文件',
    eventListLabel: '内容变更事件',
    filters: {
      facets: '筛选',
      space: '空间',
      filePath: '文件路径',
    ...
    loading: '正在加载变更...',
    reviewEmpty: '没有待审阅变更。',
    reviewEmptyHint: 'Agent 在上次审阅之后写入的内容会出现在这里；完整记录可在“全部活动”查看。',
    empty: '暂无内容变更。',
