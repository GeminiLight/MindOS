...
export const OBSIDIAN_CAPABILITY_MATRIX: ObsidianCapabilityRow[] = [
  ...
  },
  {
    ...
    host: 'Obsidian module shim',
    tests: ['obsidian-compat/integration.test.ts', 'obsidian-compat/compatibility-report.test.ts'],
    notes: 'Link text parsing strips alias and subpath components for common wiki-link targets; full Obsidian path resolution still belongs to MetadataCache.',
  },
  {
    api: 'parseLinktext',
    surface: 'metadata',
    ...
    host: 'Obsidian module shim',
    tests: ['obsidian-compat/integration.test.ts', 'obsidian-compat/compatibility-report.test.ts'],
    notes: 'Link text parsing returns path and #subpath for common wiki-link targets; it does not resolve files or headings.',
  },
  {
    ...
    host: 'Obsidian module shim',
    tests: ['obsidian-compat/integration.test.ts', 'obsidian-compat/compatibility-report.test.ts'],
    notes: 'Binary conversion is implemented as a pure ArrayBuffer/base64 transform.',
  },
  {
    ...
    host: 'Obsidian module shim',
    tests: ['obsidian-compat/integration.test.ts', 'obsidian-compat/compatibility-report.test.ts'],
    notes: 'Binary conversion is implemented as a pure base64/ArrayBuffer transform.',
  },
  {
    api: 'debounce',
    surface: 'core',
  ...
  {
    api: 'parseFrontMatterEntry',
    surface: 'metadata',
    support: 'full',
    host: 'Frontmatter shim',
    tests: ['obsidian-compat/frontmatter-entry.test.ts'],
    notes: 'String and RegExp key lookup over the parsed frontmatter object; global regexes are treated as stateless across calls.',
  },
  {
    api: 'parseFrontMatterStringArray',
    surface: 'metadata',
    support: 'full',
    host: 'Frontmatter shim',
    tests: ['obsidian-compat/frontmatter-entry.test.ts'],
    notes: 'Array values stringify entry by entry, plain strings wrap once without comma splitting, and other value types read as null.',
  },
  {
    api: 'resolveSubpath',
    surface: 'metadata',
    support: 'limited',
    host: 'Subpath resolver shim',
    tests: ['obsidian-compat/resolve-subpath.test.ts'],
    notes: 'Heading and block subpaths resolve against the shared MindOS markdown metadata parser; footnote subpaths return null because the server-tier cache does not index footnotes yet.',
  },
  {
    api: 'getAllTags',
    surface: 'metadata',
    ...
    host: 'Obsidian module shim',
    tests: ['obsidian-compat/integration.test.ts'],
    notes: 'Tag aggregation reads frontmatter tags and parsed body tag caches, normalizes # prefixes, and does not trigger a full Obsidian metadata index.',
  },
  {
    api: 'getLanguage',
    surface: 'core',
  ...
  {
    api: 'requireApiVersion',
    surface: 'core',
    support: 'limited',
    host: 'Obsidian module shim',
    tests: ['obsidian-compat/integration.test.ts'],
    notes: 'Version checks compare against the MindOS compatibility API version rather than the full installed Obsidian app version.',
  },
  {
  ...
  },
  {
    ...
    surface: 'editor',
    support: 'limited',
    host: 'Editor field shim',
    tests: ['obsidian-compat/editor-fields.test.ts'],
    notes: 'Real CodeMirror 6 StateField instance, so plugins can compose headless EditorStates; the server tier reads the editor context as null because no live editor exists.',
  },
  {
    api: 'editorEditorField',
    surface: 'editor',
    support: 'limited',
    host: 'Editor field shim',
    tests: ['obsidian-compat/editor-fields.test.ts'],
    notes: 'Real CodeMirror 6 StateField instance for headless use; the EditorView value stays null in the server tier and the browser host installs its own live field when one exists.',
  },
  {
    api: 'editorLivePreviewField',
    surface: 'editor',
    support: 'limited',
    host: 'Editor field shim',
    tests: ['obsidian-compat/editor-fields.test.ts'],
    notes: 'Real CodeMirror 6 StateField that reports live preview as inactive (false); the server tier renders no in-document preview to sync with.',
  },
  {
  ...
  },
  {
    ...
    surface: 'workspace',
    support: 'limited',
    host: 'Workspace event shim',
    tests: ['obsidian-compat/workflow-probes.test.ts'],
    notes: 'Workspace event listeners are registered and cleaned up through the compatibility host; only explicit MindOS-triggered event flows are mounted.',
  },
  {
    api: 'Workspace.trigger',
    surface: 'workspace',
    support: 'limited',
    host: 'Workspace event shim',
    tests: ['obsidian-compat/workflow-probes.test.ts'],
    notes: 'Workspace events can be triggered inside the compatibility host, but MindOS does not mirror the full native Obsidian workspace event stream.',
  },
  {
    api: 'Workspace.editor-menu',
    surface: 'workspace',
    support: 'limited',
    host: 'Workspace editor-menu probe host',
    route: '/api/obsidian-plugins',
    tests: ['obsidian-compat/workflow-probes.test.ts'],
    notes: 'MindOS can synthesize a bounded editor-menu event with tag token context for workflow probes and safe menu continuation.',
  },
  {
  ...
  },
  {
    ...
    route: '/plugins/views',
    tests: ['obsidian-compat/workspace.test.ts'],
    notes: 'Returns a compatibility leaf for recorded file/view opening; it is not a full Obsidian pane graph.',
  },
  {
    api: 'Workspace.splitActiveLeaf',
    surface: 'views',
    support: 'limited',
    host: 'Plugin view host',
    route: '/plugins/views',
    tests: ['obsidian-compat/workspace.test.ts'],
    notes: 'Creates a compatibility leaf for snapshot/open requests; it is not a full split-pane layout host.',
  },
  {
