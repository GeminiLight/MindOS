...
matcher 只回答“这个 skill 和这个 runtime 在某个 scenario 下是否匹配，以及为什么”。它不启动自动路由，也不替代 runtime 自己的权限、session、compact 或执行语义。因此当前 readiness blocker 已从 `skill-runtime-matcher` 前移到 `skill-runtime-enforcement`：turn runner 在自动选择或执行 skill 前，还需要用 matcher 结果做拦截、降级或提示。
